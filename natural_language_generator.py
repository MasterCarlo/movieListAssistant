...
def generateLLMResponse(dialogueST: DialogueStateTracker, no_movie: list[str], other_request: str) -> str:
    ...
    if len(no_movie) > 0:
        movie_list_str: str = "; ".join(no_movie)
        other_request = other_request + " Tell the user that you were not able to find any movie or series with the given title(s): " + movie_list_str + ". Ask him if those are the correct titles."
